...
const Checkout = ({order,user,addAddress,setShipAddress,placeOrder})=>{
  const blank_address= {first_name:'',last_name:'',phone:'',address1:'',address2:'',state:"",country:'',pin_code:''}
  const [address,setAddress] = useState(blank_address);

const validateAddress = (address) => {
  if(!address.first_name || !address.last_name || !address.phone || !address.address1 || !address.state || !address.country || !address.pin_code)
  {
    alert("Enter the required fiels")
}
else{
  addAddress(address);
        ...
        <form className="needs-validation" noValidate onSubmit={(e)=>{e.preventDefault();validateAddress(address)}}>
          <div className="row g-3">
            ...
            <div className="col-12">
              <label htmlFor="phone" className="form-label">Phone <span className="text-muted" /></label>
